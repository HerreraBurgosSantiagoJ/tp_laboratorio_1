...
int void_generateID( LinkedList* pLL, int ( *pIdGeter ) (void*, int*), int* newID )
{
	int returnAux = -1;
	int loopLength;
	void* pThisA;
	int auxId;
	if( pLL != NULL && pIdGeter != NULL && newID != NULL )
	{
		*newID = 0;
		returnAux = 0;
		loopLength = ll_len(pLL);
		for( int i = 0; i < loopLength; i++ )
		{
			if( ( pThisA = ll_get(pLL, i) ) == NULL ||
				pIdGeter( pThisA, &auxId ) == -1 )
			{
				returnAux = -1;
				break;
			}
